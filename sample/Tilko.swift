...
class HubManager:ObservableObject {
    ...
    let drugUrl = "https://api.tilko.net/api/v1.0/hira/hiraa050300000100"  // Drug API
    ...
    func fetchDrug(info:CertInfo) {

        let certFileURL = URL(fileURLWithPath: info.filePath).appendingPathComponent("signCert").appendingPathExtension("der")
        let keyFileURL = URL(fileURLWithPath: info.filePath).appendingPathComponent("signPri").appendingPathExtension("key")
        ...
        do {
            ...
            var request = try! URLRequest(url: drugUrl, method: .post)
